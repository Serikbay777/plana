...
Срабатывает, когда `Floor.apartments` заполнены реальной геометрией — это
делает адаптер `domain/layout_bridge.layout_to_project` (из LayoutFloor).
По старому пути (`marketing_to_project` с `floors=[]`) валидатор не находит
комнат и тихо ничего не проверяет.
"""
