...
def save_asset(project_id: str, tab: str, variant_key: str, image_bytes: bytes, floor: int = 1) -> str:
    """Сохраняет PNG на диск, возвращает относительный путь."""
    folder = ASSETS_DIR / project_id / f"floor_{floor}"
    folder.mkdir(parents=True, exist_ok=True)
    filename = f"{tab}__{variant_key}.png"
    (folder / filename).write_bytes(image_bytes)
    return f"{project_id}/floor_{floor}/{filename}"
